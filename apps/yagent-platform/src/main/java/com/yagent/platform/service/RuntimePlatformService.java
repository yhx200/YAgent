...
public class RuntimePlatformService {
    ...
    public CapabilitySearchResponse search(CapabilitySearchRequest request) {
        ...
        for (YaCapability c : capabilities) {
            double score = score(request.getQuery(), c);
        ...
        for (Scored s : top) {
            ...
            YaCapability c = s.capability();
            item.setCapabilityCode(c.getCapabilityCode());
            ...
            for (YaToolCapability m : mapping.getOrDefault(c.getCapabilityCode(), List.of())) {
                CapabilityProvider p = new CapabilityProvider();
                p.setType("TOOL");
                p.setToolId(m.getToolId());
                p.setVersion(m.getToolVersion());
                p.setToolName(m.getToolName());
                p.setInstalled(installationMapper.selectOneInstalled(request.getTenantId(), m.getToolId()) != null);
                item.getProviders().add(p);
            }
    ...
    public InstallationResolveResponse resolve(InstallationResolveRequest request) {
        InstallationResolveResponse response = new InstallationResolveResponse();
        response.setCapabilityCode(request.getCapabilityCode());
        ...
        for (YaToolCapability mapping : mappings) {
            ...
            YaTool tool = tools.get(mapping.getToolId());
            if (tool == null) continue;
            YaToolVersion version = toolVersionMapper.selectPublished(mapping.getToolId(), install.getVersion());
            if (version == null) continue;
            ToolManifest manifest = parseManifest(version.getManifestJson());
            ToolManifest.ToolFunction fn = manifest.getTools().stream().filter(x -> Objects.equals(x.getName(), mapping.getToolName())).findFirst().orElse(null);
            if (fn == null) continue;
            ResolvedTool dto = new ResolvedTool();
            dto.setToolId(tool.getToolId());
            dto.setVersion(version.getVersion());
            dto.setToolName(mapping.getToolName());
            dto.setDisplayName(tool.getDisplayName());
            dto.setDescription(StringUtils.hasText(fn.getDescription()) ? fn.getDescription() : tool.getDescription());
            dto.setRuntimeType(version.getRuntimeType());
            dto.setProtocolVersion(version.getProtocolVersion());
            dto.setEntrypoint(manifest.getRuntime().getEntry());
            dto.setInputSchema(fn.getInputSchema());
            dto.setTimeoutMs(manifest.getLimits() != null && manifest.getLimits().getTimeoutMs() != null ? manifest.getLimits().getTimeoutMs() : 10000);
            PackageInfo pkg = new PackageInfo();
            pkg.setObjectKey(version.getPackageObjectKey());
            pkg.setSha256(version.getPackageSha256());
            pkg.setSignature(version.getSignature());
            dto.setPackageInfo(pkg);
            for (YaToolPermission permission : toolPermissionMapper.selectByToolVersion(tool.getToolId(), version.getVersion())) {
                ToolPermission p = new ToolPermission();
                p.setType(permission.getPermissionType());
                p.setValue(permission.getPermissionValue());
    ...
    public PermissionEvaluateResponse evaluate(PermissionEvaluateRequest request) {
        ...
        if (installationMapper.selectOneInstalled(request.getTenantId(), request.getToolId()) == null) {
            response.setDecision("DENY");
            response.getReasons().add("Tool is not installed for tenant");
            return response;
        }
        YaTenantToolPolicy policy = policyMapper.selectAccessPolicy(request.getTenantId(), request.getToolId());
        if (policy != null && "DENY".equalsIgnoreCase(policy.getPolicyValue())) {
            response.setDecision("DENY");
            response.getReasons().add("Tenant policy denies this tool");
            return response;
        }
        ...
        if (dangerous) {
            ...
            response.getReasons().add("Tool requests shell permission");
            return response;
        }
    ...
    public void audit(AuditEventRequest request) {
        YaAuditEvent e = new YaAuditEvent();
        e.setTraceId(request.getTraceId());
        ...
        e.setCapabilityCode(request.getCapabilityCode());
        e.setToolId(request.getToolId());
        e.setToolVersion(request.getToolVersion());
        e.setToolName(request.getToolName());
        e.setResultStatus(request.getResultStatus());
        e.setDurationMs(request.getDurationMs());
        e.setDetailJson(request.getDetailJson());
        auditMapper.insert(e);
    }

    public YaToolVersion version(String toolId, String version) {
        return toolVersionMapper.selectPublished(toolId, version);
    }

    private ToolManifest parseManifest(String json) {
        try {
            return objectMapper.readValue(json, ToolManifest.class);
        } catch (Exception e) {
            throw new IllegalStateException("Invalid manifest_json", e);
        }
    }

    private double score(String query, YaCapability c) {
        String q = normalize(query);
        double s = 0;
        if (contains(q, c.getName())) s += 5;
        if (contains(q, c.getDescription())) s += 2;
        if (StringUtils.hasText(c.getKeywords())) for (String k : c.getKeywords().split("[,，]"))
            if (StringUtils.hasText(k) && q.contains(normalize(k))) s += 3;
        return s;
    }
    ...
    private boolean contains(String q, String v) {
        return StringUtils.hasText(v) && q.contains(normalize(v));
    }
