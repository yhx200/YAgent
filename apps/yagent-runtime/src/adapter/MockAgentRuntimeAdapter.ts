...
function call(name: string, args: unknown): AgentStepResult {
    return {content: null, toolCalls: [{id: `mock-${++seq}`, name, arguments: JSON.stringify(args)}]};
}
    ...
    async runStep(input: AgentStepInput): Promise<AgentStepResult> {
        ...
        if (last?.role === 'tool') {
            ...
            if (previousName === 'capability_search') {
                const code = data.items?.[0]?.capabilityCode;
                if (!code) return {content: '没有找到可用能力。', toolCalls: []};
                return call('capability_acquire', {capabilityCode: code});
            }
            if (previousName === 'capability_acquire') {
                const weather = input.tools.find(t => t.name === 'weather_query');
                if (!weather) return {content: '能力已获取，但没有发现 weather_query。', toolCalls: []};
            ...
            if (previousName === 'weather_query') {
                return {
                    content: `查询完成：${data.city}当前 ${data.temperature}${data.unit ?? '°C'}，${data.condition}。`,
                    toolCalls: []
                };
            }
        }
