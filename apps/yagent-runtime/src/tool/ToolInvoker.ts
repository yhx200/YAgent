...
export class ToolInvoker {
    ...
    async invoke(input: {
        ...
        const binding = this.registry.get(input.sessionId, input.llmName);
        if (!binding) throw new Error(`Unknown active tool: ${input.llmName}`);
        ...
        try {
            ...
            void this.audit.report({
                ...
                detailJson: JSON.stringify({argKeys: Object.keys(args)})
            });
            return result;
        } catch (error) {
            void this.audit.report({
                ...
                resultStatus: 'FAILED',
                durationMs: Date.now() - start,
                detailJson: JSON.stringify({error: errorMessage(error)})
            });
            throw error;
        }
    }
