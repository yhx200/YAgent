...
export class ToolRegistry {
    ...
    register(sessionId: string, binding: Omit<ToolBinding, 'llmName'>): ToolBinding {
        const registry = this.bySession.get(sessionId) ?? new Map<string, ToolBinding>();
        this.bySession.set(sessionId, registry);
        let base = sanitize(binding.toolName), name = base, i = 2;
        while (registry.has(name) && registry.get(name)?.toolName !== binding.toolName) {
            name = `${base}_${i++}`;
        }
    ...
    get(sessionId: string, llmName: string) {
        return this.bySession.get(sessionId)?.get(llmName);
    }
