...
export interface MaterializedTool { packageDir:string; entrypoint:string; timeoutMs:number; }
...
export class ToolMaterializer {
  ...
  async materialize(tool:ResolvedTool):Promise<MaterializedTool>{
    ...
    const response=await fetch(ticket.url); if(!response.ok)throw new Error(`Tool package download failed ${response.status}`);
    ...
    const tmp=dir+`.tmp-${Date.now()}`; await fs.rm(tmp,{recursive:true,force:true}); await fs.mkdir(tmp,{recursive:true});
    const zipPath=path.join(tmp,'package.ytool'); await fs.writeFile(zipPath,data);
    new AdmZip(zipPath).extractAllTo(tmp,true); await fs.rm(zipPath,{force:true});
    ...
    await fs.writeFile(path.join(tmp,'.verified'),JSON.stringify({sha256:actual,verifiedAt:new Date().toISOString()}));
    await fs.rm(dir,{recursive:true,force:true}); await fs.rename(tmp,dir);
    return {packageDir:dir,entrypoint:manifest.runtime.entry,timeoutMs:manifest.limits?.timeoutMs??tool.timeoutMs??10000};
  }
}
