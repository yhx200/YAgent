...
export class AgentRuntime {
  ...
  async run(context:{tenantId:number;userId:number;sessionId:string},userMessage:string):Promise<string>{
    ...
    for(let round=0;round<10;round++){
      ...
      if(!result.toolCalls.length){const content=result.content??'';this.sessions.addMessage(context.sessionId,{role:'assistant',content});return content;}
      this.sessions.addMessage(context.sessionId,{role:'assistant',content:result.content,toolCalls:result.toolCalls});
      for(const call of result.toolCalls){
        let output:unknown;
        try{
          if(call.name.startsWith('capability_')) output=await this.tools.invokeBootstrap({...context,name:call.name,argumentText:call.arguments});
          else output=await this.invoker.invoke({...context,llmName:call.name,argumentText:call.arguments});
        }catch(error){output={success:false,error:error instanceof Error?error.message:String(error)};}
        this.sessions.addMessage(context.sessionId,{role:'tool',content:JSON.stringify(output),toolCallId:call.id});
      }
    }
